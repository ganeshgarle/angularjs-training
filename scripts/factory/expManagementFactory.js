app.factory('expManagementFactory', function(expneseMgtService) {
  ...
  factory.deleteEntry = function(scope,obj,type,ctrl){
    if(type == "Expense"){
        scope.expneseServiceData.currentBalance = parseInt( scope.expneseServiceData.currentBalance ) + parseInt( obj.amount );
    }else{
        scope.expneseServiceData.currentBalance = parseInt( scope.expneseServiceData.currentBalance ) - parseInt( obj.amount );
    }
  ...
  factory.editEntry = function(scope,type){
      for( var i = 0; i < scope.expneseServiceData.expensesData.length; i++ ){
